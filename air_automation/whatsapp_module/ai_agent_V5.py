...
2. **Generate replies**: I might ask you to draft a reply to a contact. Compose an appropriate response and ask for my confirmation before finalizing. If I provide a specific reply, use it directly. Format your responses as follows. strictly follow this format:
---------
...
def ai_query_unread_messages(query):
    ...
    try:
        ...
        if "message sent successfully to" in ai_response.lower():
            save_message_to_file(ai_response)  # Save the response directly
...
# Updated save_message_to_file function
def save_message_to_file(message_content):
    # Replace "current time" with the actual current time in the message content
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    formatted_message = re.sub(r"(Sent Time:).+", f"\\1 {current_time}", message_content)

    # Extract Contact Name, Sent Time, and Message from the response
    contact_name = re.search(r"Contact Name/Phone Number:\s*(.+)", formatted_message)
    sent_time = re.search(r"Sent Time:\s*(.+)", formatted_message)
    message_text = re.search(r"Message:\s*(.+)", formatted_message)

    # Ensure all required fields are found before saving
    if contact_name and sent_time and message_text:
        # Format the output in the specified structure
        clean_message = (
            f"Contact Name/Phone Number: {contact_name.group(1)}\n"
            f"Sent Time: {current_time}\n"
            f"Message: {message_text.group(1)}"
        )
        
        # Append the formatted message to send_whatsapp.txt
