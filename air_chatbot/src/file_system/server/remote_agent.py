...
def agent_query():
    try:
        ...
        # Check connection status using actual connection state
...
def run_agent_interface(query=None):
    ...
    if query is None:
        ...
        try:
            ...
            state = get_connection_state()
            if not state["is_connected"]:
                return "No active connection to a PC. Please establish a connection first to perform file operations."
...
def heartbeat():
    """Heartbeat function that runs every 30 seconds to check and reconnect if needed"""
    while True:
        try:
            # Check if already connected using the actual connection state
            state = get_connection_state()
            if state["is_connected"]:
                ...
                try:
                    ssh, _ = ensure_connection()
                    if verify_connection(ssh):
                        logging.info("Heartbeat: Connection verified and working")
                        time.sleep(30)
                        continue
                    else:
                        logging.info("Heartbeat: Connection state says connected but verification failed")
                except:
                    logging.info("Heartbeat: Connection state says connected but verification failed")
            
            logging.info("Heartbeat: Not connected, checking for reconnection...")
            
            # Check if connection_config.json exists
            if not os.path.exists('connection_config.json'):
                logging.info("Heartbeat: No connection_config.json found")
                time.sleep(30)
                continue
            ...
            if not config.get('current_connection', {}).get('hostname'):
                logging.info("Heartbeat: No hostname in connection config")
                time.sleep(30)
                continue
            
            # Find the latest connection file (exclude connection_config.json)
            connection_files = [f for f in os.listdir('.') if f.startswith('connection_') and f.endswith('.json') and f != 'connection_config.json']
            if not connection_files:
                logging.info("Heartbeat: No connection files found")
                time.sleep(30)
                continue
            
            # Get the latest connection file (most recent modification time)
            latest_file = max(connection_files, key=lambda f: os.path.getmtime(f))
            logging.info(f"Heartbeat: Using latest connection file: {latest_file}")
            
            # Load connection details from the latest file
            with open(latest_file, 'r') as f:
                connection_data = json.load(f)
            
            # Extract required data
            ssh_command = connection_data.get('ssh_command')
            device_id = connection_data.get('device_id')
            device_name = connection_data.get('device_name')
            password = config['current_connection']['password']
            
            if not all([ssh_command, device_id, password]):
                logging.info(f"Heartbeat: Missing required data - ssh_command: {bool(ssh_command)}, device_id: {bool(device_id)}, password: {bool(password)}")
                time.sleep(30)
                continue
            
            logging.info(f"Heartbeat: Attempting to reconnect to {device_name}")
            
            # Prepare data for connection
            data = {
                'ssh_command': ssh_command,
                'password': password,
                'os_type': connection_data.get('os_type', 'unknown'),
                'device_id': device_id
            }
            
            # Make internal call to connect endpoint
            with app.test_client() as client:
                response = client.post('/api/pc/connect', json=data)
                
                if response.status_code == 200:
                    result = response.get_json()
                    if result.get('status') == 'connected':
                        logging.info(f"Heartbeat: Successfully reconnected to {device_name}")
                    else:
                        logging.warning(f"Heartbeat: Connection failed - {result.get('error')}")
                else:
                    logging.warning(f"Heartbeat: Connection request failed with status {response.status_code}")
...
if __name__ == "__main__":
    ...
    heartbeat_thread.daemon = True  # This thread will exit when the main thread exits
    heartbeat_thread.start()
    print("Heartbeat thread started - will check for reconnection every 30 seconds")
